...
export function ProductCard({ listing }: ProductCardProps) {
  ...
  return (
    <li className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4">
      <Link className="block overflow-hidden rounded-md bg-slate-100" to={detailPath}>
        {thumbnail
      ...
      <div className="flex flex-1 flex-col gap-1">
        <h2 className="text-lg font-semibold leading-tight"><Link className="rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-700" to={detailPath}>{listing.legoProduct.title}</Link></h2>
